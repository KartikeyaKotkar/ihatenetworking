...
import { portQuizQuestion } from "@/lib/learn";
...
export default function Quiz() {
  const [q, setQ] = useState<Question>(() => portQuizQuestion());
  ...
  function check(): void {
    ...
    posthog.capture("quiz_answer_checked", { quiz: "port-number", correct: isCorrect });
  }

  function newQuestion(): void {
    setQ(portQuizQuestion());
    setPicked(null);
    setChecked(false);
  }

  function reset(): void {
    posthog.capture("quiz_reset", { quiz: "port-number", score, total });
    setScore(0);
    setTotal(0);
    setQ(portQuizQuestion());
    setPicked(null);
