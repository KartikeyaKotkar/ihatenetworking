...
export default function Calculator() {
  ...
  async function onSubmit(e?: React.FormEvent) {
    ...
    try {
      const r = await fetch(`/api/dns?` + new URLSearchParams({ name: name.trim(), type: "A" }));
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      <p className="mt-2 text-xs text-gray-500">IPv4 addresses only. Need IPv6? Use AAAA via <a className="underline" href="/dns-lookup">DNS Lookup</a>.</p>
      <div className="mt-3 flex gap-2">
        <button type="submit" disabled={loading} className="rounded-md bg-emerald-500 px-4 py-1.5 text-xs font-semibold text-black hover:bg-emerald-400 disabled:opacity-50">{loading ? "Looking up…" : "Look up A record"}</button>
      ...
      <div className="mt-4">
        ...
        {answer && (
          <div className="rounded-lg bg-black/40 p-4">
            ...
            <div className="mt-2">
              <p className="text-xs text-gray-400">IPv4 addresses ({answer.values.length})</p>
